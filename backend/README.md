...
| `GET` | `/charts` | `read:analytics` | same | `{ skillsGap, certificationTrend, certificationTrendSeries, alumniTotalsByYear, employmentByIndustry, commonJobTitles, topEmployers, topCourseProviders, geographicDistribution }`. `skillsGap` items include `{ label, percentage, severity }` where `severity` is derived from the percentage of the total alumni pool: `critical` (≥70%), `significant` (≥50%), `emerging` (≥20%), `monitor` (<20%). `certificationTrendSeries` is grouped by certification + year; `alumniTotalsByYear` enables percentage rendering when totals are available. |
