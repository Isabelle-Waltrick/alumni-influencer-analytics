    ...
    <section className="space-y-4">
      ...
      {charts && (
        <div id="charts-grid" className="grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-lg border bg-white p-4"><h3 className="mb-3 text-sm font-semibold">Top Issuing Bodies (Pie)</h3><Pie data={{ labels: charts.topIssuingBodies.map((x) => x.label), datasets: [{ data: charts.topIssuingBodies.map((x) => x.value) }] }} /></div>
